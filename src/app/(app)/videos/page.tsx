...
export default function VideosPage() {
  ...
  useEffect(() => {
    async function fetchModels() {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          setModels((data.data || []).filter((m: ModelConfig) =>
            (m.capability_metadata as Record<string, unknown>)?.media_type === 'video'
          ));
        }
      } catch { /* non-critical */ }
    }
  ...
  const toggleFavorite = async (assetId: string, currentFavorite: boolean) => {
    try {
      const res = await fetchWithTimeout(`/api/v1/videos/${assetId}`, {
        method: 'PATCH',
        headers: {
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 md:mb-6 gap-3">
        <h1 className="text-lg font-semibold text-[var(--color-text)]">视频库</h1>
            ...
            >
              {asset.url || asset.thumbnail_url ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-[var(--color-text-subtle)]">
                  <span className="text-xs">视频 {asset.id.slice(0, 6)}</span>
                </div>
              )}
              {/* Play icon overlay */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
                <div className="w-9 h-9 rounded-full bg-black/50 flex items-center justify-center">
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="white">
                    <path d="M3 1.5L11 7L3 12.5V1.5Z" />
                  </svg>
                </div>
              </div>
              {/* Duration tag */}
          ...
          <div className="md:hidden space-y-2">
            {assets.map((asset) => (
              <div key={asset.id} className="flex items-center gap-3 p-3 border border-[var(--color-border)] rounded-[var(--radius-md)] bg-[var(--color-surface)]">
                <div className="w-16 h-12 rounded-[var(--radius-sm)] bg-[var(--color-surface-subtle)] flex items-center justify-center flex-shrink-0 relative overflow-hidden">
                  {asset.thumbnail_url ? (
                    <img src={asset.thumbnail_url} alt="" className="w-full h-full object-cover" loading="lazy" />
                  ) : null}
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                    <svg width="10" height="10" viewBox="0 0 14 14" fill="white" opacity="0.8">
                      <path d="M3 1.5L11 7L3 12.5V1.5Z" />
                    </svg>
                  </div>
                </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[var(--color-surface-subtle)] border-b border-[var(--color-border)]">
                  ...
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-[var(--color-text-muted)]">分辨率</th>
      ...
      {total > pageSize && (
        <div className="flex items-center justify-between mt-4">
          <span className="text-xs text-[var(--color-text-muted)]">共 {total} 个</span>
          <div className="flex gap-1">
