...
export const userQuotas = pgTable(
  "user_quotas",
  {
    ...
    daily_video_limit: integer("daily_video_limit").notNull().default(10),
    monthly_video_limit: integer("monthly_video_limit").notNull().default(50),
    max_videos_per_request: integer("max_videos_per_request").notNull().default(1),
    created_at: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
    updated_at: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => [
...
export const generationAssets = pgTable(
  "generation_assets",
  {
    ...
    model_code: varchar("model_code", { length: 50 }),
